{%- set schema_name,
        ...
        shop_table_name
        = 'shopify_raw_ca', 'order', 'order_line', 'order_line_refund', 'shop' -%}
...
{%- set order_line_raw_tables = dbt_utils.get_relations_by_pattern('shopify_raw_ca%', 'order_line') -%}
{%- set order_line_refund_raw_tables = dbt_utils.get_relations_by_pattern('shopify_raw_ca%', 'order_line_refund') -%}
{%- set shop_raw_tables = dbt_utils.get_relations_by_pattern('shopify_raw_ca%', 'shop') -%}

WITH 
    {% if var('sho_ca_currency') == 'USD' -%}
    shop_raw_data AS 
    ...
    {%- set conversion_rate = 1 if var('sho_ca_currency') != 'USD' else 'conversion_rate' %}    
